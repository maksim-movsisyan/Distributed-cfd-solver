...
inline void slip_wall_kernel(std::span<double* const> q,
    ...
    for (std::size_t face_idx = beg; face_idx < end; ++face_idx) {
        ...
        numerics::boundary::apply_slip_component(vx[gh], vy[gh], vz[gh],
                                                 vx[in], vy[in], vz[in],
                                                 nx, ny, nz);
...
inline void slip_wall_grad_kernel(std::span<const double* const> q,
    ...
    for (std::size_t face_idx = beg; face_idx < end; ++face_idx) {
        ...
        numerics::boundary::apply_neumann_gradient(gx[0][gh], gy[0][gh], gz[0][gh],
        ...
        for (std::size_t d = 0; d < 3; ++d) {
            ...
            numerics::boundary::apply_slip_gradient(gx[v][gh], gy[v][gh], gz[v][gh],
