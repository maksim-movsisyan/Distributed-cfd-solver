...
/**
 * @class SymmetryBC
 * @brief Symmetry plane boundary condition for Incompressible solver.
 */
class SymmetryBC final : public IncompressibleBC {
public:
    SymmetryBC(std::string zone,
               const LocalIndex fbeg,
               const LocalIndex fend)
        : IncompressibleBC(std::move(zone), fbeg, fend) {}

    void update_ghost_cells(std::span<double* const> q,
                            const mesh::MeshPart& mesh) const override {
        assert(q.size() == 4 && "SymmetryBC requires 4 variables [p, u, v, w]");
        bc::physical::kernels::symmetry_kernel<4>(q, mesh, this->m_begin, this->m_end);
    }
    ...
    void update_ghost_cells_grad(std::span<const double* const> q,
        ...
        bc::physical::kernels::symmetry_grad_kernel<4>(q, gx, gy, gz, mesh, this->m_begin, this->m_end);
    }
    ...
    [[nodiscard]] bc::physical::BCType kind() const noexcept override { 
        return bc::physical::BCType::Symmetry; 
    }
};
