...
namespace cfd::solver::compressible::eos {
...
[[nodiscard]] inline double specific_internal_energy(const double U[5]) noexcept {
...
template <EquationOfStatePolicy EOS>
[[nodiscard]] inline double pressure(const EOS& eos, const double U[5]) noexcept {
    const double rho = U[0];
...
template <EquationOfStatePolicy EOS>
[[nodiscard]] inline double temperature(const EOS& eos, const double U[5]) noexcept {
    const double rho = U[0];
    const double e = specific_internal_energy(U);
...
template <EquationOfStatePolicy EOS>
[[nodiscard]] inline double sound_speed(const EOS& eos, const double U[5]) noexcept {
    const double rho = U[0];
...
template <EquationOfStatePolicy EOS>
[[nodiscard]] inline double total_enthalpy(const EOS& eos, const double U[5]) noexcept {
    const double p = pressure(eos, U);
    return (U[4] + p) / U[0];
}
