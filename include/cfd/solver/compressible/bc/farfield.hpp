...
inline void farfield_kernel(std::span<double* const> q,
    ...
    for (std::size_t face_idx = beg; face_idx < end; ++face_idx) {
        ...
        if (is_supersonic_outflow) {
            ...
            numerics::boundary::apply_extrapolate_value(vx[gh],  vx[in]);
            numerics::boundary::apply_extrapolate_value(vy[gh],  vy[in]);
            numerics::boundary::apply_extrapolate_value(vz[gh],  vz[in]);
            numerics::boundary::apply_extrapolate_value(tmp[gh], tmp[in]);
        } else {
            numerics::boundary::apply_dirichlet_value(prs[gh], prs[in], pb);
            numerics::boundary::apply_dirichlet_value(vx[gh],  vx[in],  vxb);
            numerics::boundary::apply_dirichlet_value(vy[gh],  vy[in],  vyb);
            numerics::boundary::apply_dirichlet_value(vz[gh],  vz[in],  vzb);
            numerics::boundary::apply_dirichlet_value(tmp[gh], tmp[in], Tb);
