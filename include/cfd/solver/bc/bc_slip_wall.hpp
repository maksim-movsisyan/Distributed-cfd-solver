...
inline void slip_wall_kernel(fields::PrimitiveView<double> s, const mesh::MeshPart& m,
    ...
    for (std::size_t face_idx = static_cast<std::size_t>(fbeg);
        ...
        // ==== Reflect velocity vector (Symmetry / Slip: u_n = 0) ====
        apply_slip_component_bc(s.vx[gh], s.vy[gh], s.vz[gh],
                                s.vx[in], s.vy[in], s.vz[in],
                                nx, ny, nz);

        // ==== Extrapolate scalars from inner cell ====
...
inline void slip_wall_grad_kernel(fields::PrimitiveView<const double> s,
    ...
    for (std::size_t face_idx = static_cast<std::size_t>(fbeg);
        ...
        // ==== temperature grad (zero normal gradient / adiabatic: dT/dn = 0) ====
...
/**
 * @class SlipWallBC
 * @brief Slip wall boundary condition implementation (inviscid wall).
 */
class SlipWallBC : public BoundaryCondition {
public:
    /** @brief SlipWallBC constructor */
    SlipWallBC(std::string zone, LocalIndex fbeg, LocalIndex fend)
        : BoundaryCondition(std::move(zone), fbeg, fend) {}

    /** @brief SlipWallBC apply implementation */
    void apply(fields::PrimitiveView<double> state,
               const mesh::MeshPart& mesh) const override {
        slip_wall_kernel(state, mesh, m_begin, m_end);
    }

    /** @brief SlipWallBC apply gradient implementation */
    void apply_grad(fields::PrimitiveView<const double> state,
                    fields::PrimitiveGradView<double> state_grad,
                    const mesh::MeshPart& mesh) const override {
        slip_wall_grad_kernel(state, state_grad, mesh, m_begin, m_end);
    }

    BCType kind() const override { return BCType::SlipWall; }
};
