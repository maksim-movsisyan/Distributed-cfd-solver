...
inline void no_slip_wall_heat_flux_kernel(std::span<double* const> q,
    ...
    for (std::size_t face_idx = beg; face_idx < end; ++face_idx) {
        ...
        numerics::boundary::apply_dirichlet_value(vx[gh], vx[in], vx_w);
        numerics::boundary::apply_dirichlet_value(vy[gh], vy[in], vy_w);
        numerics::boundary::apply_dirichlet_value(vz[gh], vz[in], vz_w);
        ...
        numerics::boundary::apply_neumann_value(tmp[gh], tmp[in], tmp_grad_wall, rcfn);
...
inline void no_slip_wall_heat_flux_grad_kernel(std::span<const double* const> q,
    ...
    for (std::size_t face_idx = beg; face_idx < end; ++face_idx) {
        ...
        // 1. Pressure gradient extrapolation (zero-gradient)
        numerics::boundary::apply_extrapolate_gradient(gx[0][gh], gy[0][gh], gz[0][gh],
                                                       gx[0][in], gy[0][in], gz[0][in]);

        // 2. Velocity gradient (Dirichlet fixed value at wall)
        ...
        // 3. Temperature gradient (Neumann fixed normal gradient)
        numerics::boundary::apply_neumann_gradient(gx[4][gh], gy[4][gh], gz[4][gh],
