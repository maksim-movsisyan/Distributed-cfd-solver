 ...
 */
class VelocityInletBC final : public IncompressibleBC {
public:
    VelocityInletBC(std::string zone,
                    const LocalIndex fbeg,
                    const LocalIndex fend,
                    const bc::physical::SubsonicInletParams& p)
    ...
    void update_ghost_cells(std::span<double* const> q,
        ...
        assert(q.size() == 4 && "VelocityInletBC requires variables [p, u, v, w]");
        bc::physical::kernels::subsonic_inlet_kernel<4>(q, mesh, this->m_begin, this->m_end, m_p);
    }
    ...
    void update_ghost_cells_grad(std::span<const double* const> q,
        ...
        bc::physical::kernels::subsonic_inlet_grad_kernel<4>(q, gx, gy, gz, mesh, this->m_begin, this->m_end, m_p);
    }
    ...
    [[nodiscard]] bc::physical::BCType kind() const noexcept override { return bc::physical::BCType::SubsonicInlet; }

private:
    bc::physical::SubsonicInletParams m_p;
};
